function [hAxes, ctrs1, ctrs2, F] = dscatter(X,Y, varargin)
...
%
%   DSCATTER(...,'LOGXLOGY',true) uses a log scale for both x and y yaxis.
%
%   Examples:
%
...
if nargin > 2
    ...
    okargs = {'smoothing','bins','plottype','logy','marker','msize','filled','logxlogy','xlims', 'ylims'};
    for j=1:2:nargin-2
        pname = varargin{j};
        pval = varargin{j+1};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% added 4.4.2013
% if the histogram turns out to be one dimensional, plot it differently.
if min(nbins) == 1
    plot(10 .^ ctrs1, H, 'b-', 'LineWidth', 2);
    set(gca, 'xscale', 'log');
    xl = [roundto1digit(10 ^ minx, -1), roundto1digit(10 ^ maxx, 1)];
    set(gca, 'XLim', xl)
    return
end
% end of addition
...
okTypes = {'surf','mesh','contour','image','scatter', 'scatcont', 'hist2'};
...
elseif length(k)>1
    ...
    switch(k)
        ...
        case 3 %'contour'
            [~, h] = contour(ctrs1,ctrs2,F, 'LineWidth', 1.3);
        case 4 %'image'
            nc = 256;
            F = F./max(F(:));
        ...
        case 6 %'scatcont'
            ...
            hold on
            ff = sort(F(:));
            [~, c1] = contour(ctrs1, ctrs2, F, max(ff(cumsum(ff) < max(cumsum(ff)) / 20)));
            [~, c2] = contour(ctrs1, ctrs2, F, max(ff(cumsum(ff) < max(cumsum(ff)) / 20)));
            set(c1, 'LineWidth', 4, 'LineStyle', '-', 'Color', 'w')
            set(c2, 'LineWidth', 2, 'LineStyle', '-', 'Color', 'k')
        case 7
            hh = hist3([X, Y], {xbins, ybins});
            hh = hh ./ repmat((sum(sum(hh, 1), 2)), [numel(xbins), numel(ybins)]);
            hh(hh < max(hh(:) / 100)) = NaN;
...
if logxlogy
    ...
    xl = [roundto1digit(10 ^ minx, -1), roundto1digit(10 ^ maxx, 1)];
    yl = [roundto1digit(10 ^ miny, -1), roundto1digit(10 ^ maxy, 1)];
...
function V = roundto1digit(V, updown)
in = 10 .^ (-10 : 10);
m = find((in - V) < 0, 1, 'last');
if updown == -1
    V = floor(V / in(m)) * in(m);
elseif updown == 1
    V = ceil(V / in(m)) * in(m);
